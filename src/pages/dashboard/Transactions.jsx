...
const Transactions = () => {
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      ...
      const approveEvent = await contract.queryFilter(
        approveFilter,
        deploymentBlockNumber,
        "latest"
      );
            ...
            <section className="text-[#0F160F] flex lg:flex-row md:flex-row flex-col justify-between">
              {purchase?.length === 0 ? (
                ...
                purchase?.map((p, index) => {
                  ...
                  const isApproved = approved.length > 0 && approved.some((item, purchaseID) => item.id === p.id && item.address === address && purchaseID === index);
                  return (
                    <div
                      key={index}
                      ...
                      {userPurchase ? (
                        <div>
                          ...
                          <p className="flex justify-between my-4 font-bold">
                            Price{" "}
                            <span>{formatUnits(userPurchase.price)}AMB</span>{" "}
                          </p>
                        </div>
                      ) : (
                        <p>Product details not available.</p>
                      )}
                      {(!approved.length || !isApproved) && <ApprovePayment key={p.id} id={p.id} index={index} />}
                    </div>
                  );
                })
              )}
                    ...
                    >
                      {userApproval ? (
                        <div>
                          ...
                          <p className="flex justify-between my-4 font-bold">
                            Price{" "}
                            <span>{formatUnits(userApproval.price)}AMB</span>{" "}
                          </p>
                        </div>
                      ) : (
                        <p>Product details not available.</p>
